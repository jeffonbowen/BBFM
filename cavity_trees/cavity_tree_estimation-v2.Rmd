...
```
# Introduction
The loss of suitable trees for cavity nesting birds may be relevant for the Site C offset accounting framework. Data on decaying trees was collected at select locations as part of the Breeding Bird Follow-up Monitoring program. Decaying trees are often referred to as "wildlife trees". The wildlife tree data can be used estimate the density of wildlife trees by habitat type and then extrapolated to the entire project footprint using the ecosystem mapping data. 

This document first describes the estimation of the number of wildlife per hectare for various habitat types and then extrapolates to the entire Site C footprint.  
