...
The loss of suitable trees for cavity nesting birds may be relevant for the Site C offset accounting framework. Data on decaying trees was collected at select locations as part of the Breeding Bird Follow-up Monitoring program. Decaying trees are often referred to as "wildlife trees" in BC. The wildlife tree data can be used to estimate the density of wildlife trees by habitat type and then extrapolated to the entire project footprint using the ecosystem mapping data.

This document first describes the estimation of the number of wildlife trees per hectare for various habitat types and then extrapolates to the entire Site C footprint to generate a total count.
...
**SN50**\
Approximate total number of dead standing trees \>15cm dbh within 50m of plot centre. Trees not identified to species. Collected at all survey locations since 2017 (459 locations to date).

**SN11**\
Species by decay class in SIVI plot, \>15cm dbh. The "11" in SN11 refers to 11.3m radius = \~400m2, the area of a SIVI plot. This was only collected in 2018 and 2019 at woodpecker survey stations (318 locations), so there are fewer locations than SN50.
...
There are more notes about the data in the first tab of the "snag\_data.xlsx" workbook.
...
All data is in "snag\_data.xlsx".
...
tree_dat %>% 
  ...
  labs(subtitle = "Total Number of Wildlife Trees by Structural Stage and Stand Composition",
...
tree_dat %>% 
  ...
  labs(subtitle = "Total Number of Wildlife Trees by Structural Stage and SMR",
       y = "Total number of wildlife trees per plot")
```
...
Fit GLM with negative binomial distribution. Fitting all possible variations of the three explanatory variables. Table shows ranking by AICc.
...
Best model has SMR and Structural Stage. Though not very different from model with all three variables. It appears that stand composition has little influence on total number of snags.
...
**Predict Wildlife Tree Density Means**
...
total_trees_plot <- means_total_trees %>% 
  ...
  labs(subtitle = "Estimated Number of Wildlife Trees")+
