...
All data is in the accompanying file "wildlife\_tree\_data.xlsx".
...
The section is the same analysis as completed, except fir decaying deciduous trees only.
...
The model with the lowest AICc is the full model with SMR, stand composition and structural stage. Models with interactions were attempted but could not be fit as there are not enough samples in the various combinations of variables to model interactions.
...
It is hard to visualize the effect of three variables in a two-dimensional plot. The plot is shown twice, once for deciduous stands and the second for coniferous stands.
