...
## CONI
  ...
  CONIdat <- CONIdat %>% rename(Longitude = coords.x1, Latitude = coords.x2) 
  ...
  collapsible = TRUE,
    ...
    tabPanel("Common Nighthawk Surveys",
      ...
      div(class="outer", tags$head(includeCSS("styles.css")),
        ...
        leafletOutput("CONImap", width = "100%", height = "100%"),
        ...
        h4("Breeding Bird Follow-up Monitoring Program"),
        h4("Common Nighthawk"),
        p("This tool displays the location of Common Nighthawk detected during baseline surveys in 2010 and 2012 
           and follow-up monitoring in 2018 and 2019.")
        )
      )
    )
)
...
server <- function(input, output) {
  ...
  # Create woodpecker map
    # Reactive expresssion to subset data based on user species selection
    woospobs <- reactive({subset(wooobs, wooobs$`English Name`== input$woosp)})

    # Render basemap with stations, but observations not added yet.
    # Adding stations here forces map to proper extent and zoom level on open.
    ...
    output$CONImap <- renderLeaflet({
      basemap %>%
        # Map station data
        addCircleMarkers(lng=~Longitude, lat=~Latitude, data=CONIdat,
                         #popup = ~htmlEscape(CONIdat$'StationID'),
                         labelOptions(sticky=FALSE),
                         radius = 1.3, color = "blue",
                         group = "Point Count Locations",
                         stroke = TRUE) %>% 
        addLegend("bottomright",
                  labels = "Survey Location",
                  group = "Point Count Locations",
                  colors = "blue",
                  opacity = 1,
                  title = "Survey Locations") %>%
        # Add observations
        addCircleMarkers(lng=~Longitude, lat=~Latitude,
                         data=subset(CONIdat, CONIdat$'Detect Type' %in% c("Wing Booms", "Calls Only")),
                         labelOptions(sticky=FALSE),
                         color = "red",
                         fillOpacity = 1,
                         radius = 1,
