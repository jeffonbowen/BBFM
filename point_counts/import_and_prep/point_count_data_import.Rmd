...
surveys <- read_excel(here("point_counts", "data_raw", paste0(fname)), 
                      "Surveys") %>% 
  rename(StationID = 'Sample Station Label') %>%  +
  select(StationID, Visit, Date, Time, `Survey Duration`, `Inventory Type`) %>% 
...
BCbirds <- read_excel(here("point_counts", "data_reference", 
  ...
  select(`English Name`, `Scientific Name`, `Species Code`, Order, 
         `BC List`, COSEWIC, `SARA Status`) %>% 
...
-   Need to create two versions:
    ...
    -   Count in 5 minutes. This standardizes to 5 mins for analyses when accounting for different durations is difficult.
...
write_csv(song_wide, here("point_counts", "data_processed", "song_wide_100m.csv"))
```
...
write_csv(song_wide, here("point_counts", "data_processed", "song_wide_5min_100m.csv"))
