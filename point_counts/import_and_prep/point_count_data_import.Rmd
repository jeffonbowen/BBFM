...
QA That way the visits can be properly renumbered (although that really only matter for the visit analysis)
...
Clean up temp files before next step.
...
-   Need to create two versions:
    ...
    -   Count in 5 minutes. This standardizes to 5 mins for analyses when accounting for differing durations is difficult.

-   Since some survey years were fixed radius and some unlimited, must select only the obs =\< 100 when using data for all years. If using the 2016 onward data, can use unlimited.
...
obs_filter_wide <- obs %>% 
  ...
  summarise(count5 = sum(count5)) %>% 
  pivot_wider(names_from = "SpCode", values_from = count5, values_fill = 0)
    
  
```

Create full dataset by joining all tables and then pivot wider.

```{r}
alldat_long <- obs_filter %>% 
  inner_join(surveys, by = c("StationID", "Visit"))
...
Filter data for songbirds, less than 100m and calculate 5-min counts.

get rid of flyovers as well
...
temp <- left_join(surveys, songbirds_wide, by = c("StationID", "Visit")) 
temp

## *** Problem. Surveys were renumberd? and now dont join
