---
title: "Community Analysis Using Site C Bird Data"
output:
  html_notebook:
    toc: TRUE
---
...
4. Community strucuture exploration using ordination and cluster analysis
...
Cumulative number of species with each successtive survey.
Then take mean.
...
Does habtiat need be accounted for? Yes, because there will be a difference in number of samples by habitat. Or is that true? Would be good to do at least  Forest/Open as there may be a difference in detectibility.
...
# Add a flag to to each record to indicate if t should be included in the total
# visit summary. 
MaxVisits <- obs.5min %>% 
  group_by(StationID) %>% 
...
# GLM to account for survey differences. Can account for year and habitat. 
# Can't account for survey timing within a year becasue data is pooled.  
...
fit.x <- goodfit(dat$Richness, type = c("poisson", "binomial", "nbinomial"))
summary(fit.x)
rootogram(fit.x)
...
m.full <- glm.nb(Richness ~ Visits + BHC7 + ForestOpen + MonYear, dat = dat)
summary(m3)
x <- dredge(m.full)
x
y <- tidy(x)
...
library(visreg)       # What does this do?
visreg(bestmod)
