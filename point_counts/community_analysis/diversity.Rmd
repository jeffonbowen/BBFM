...
Notes from https://noamross.github.io/gams-in-r-course:
The number of basis functions (k) and the smoothing parameters (sp) interact to control the wiggliness of a smooth function. 

```{r paged.print=TRUE}
library(mgcv)

divdat$MonYear <- divdat$MonYear + 2006
divdat$Easting <- divdat$Easting / 100000
divdat$Northing <- divdat$Northing / 100000

gam1 <- gam(richness ~ 
              s(MonYear, k = 9, sp = 0.03) + 
...
plot(gam1, residuals = FALSE, select = 1, se = TRUE, shade=FALSE)
