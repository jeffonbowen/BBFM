## Total  Abundance Analysis of the Songbird Data (Top) ====
...
env <- dplyr::select(dat, Station:dur, Sample, year, JDAY, BHC7, BHC16, MonYear, xEasting, xMonYear)
...
abun <- diversityresult(sp.v, index = "abundance", method = "each site", sortit = FALSE, digits = 6)
divdat <- bind_cols(env, rich, shan, abun, simp)
...
#or
ggplot(divdat, aes(x=abundance)) + geom_histogram(binwidth = 1)
...
pois2 <- glmmTMB(richness ~ BHC16 + (1|Station), data = divdat, family = (poisson("log")))
summary(pois2)

pois3 <- glmmTMB(richness ~ xEasting + (1|Station), data = divdat, family = (poisson("log")))
summary(pois3)

pois4 <- glmmTMB(richness ~ MonYear + BHC16 + (1|Station), data = divdat, family = (poisson("log")))
summary(pois4)

pois5 <- glmmTMB(richness ~ MonYear + xEasting + (1|Station), data = divdat, family = (poisson("log")))
summary(pois5)

pois6 <- glmmTMB(richness ~ MonYear + xEasting + BHC16 + (1|Station), data = divdat, family = (poisson("log")))
summary(pois6)

pois7 <- glmmTMB(richness ~ MonYear + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois7)

pois8 <- glmmTMB(richness ~ BHC16 + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois8)

pois9 <- glmmTMB(richness ~ xEasting + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois9)

pois10 <- glmmTMB(richness ~ MonYear + BHC16 + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois10)

pois11 <- glmmTMB(richness ~ MonYear + xEasting + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois11)

pois12 <- glmmTMB(richness ~ MonYear + xEasting + BHC16 + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois12)

pois13 <- glmmTMB(richness ~ MonYear + (1|JDAY) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois13)

pois14 <- glmmTMB(richness ~ BHC16 + (1|JDAY) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois14)

pois15 <- glmmTMB(richness ~ xEasting + (1|JDAY) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois15)

pois16 <- glmmTMB(richness ~ MonYear + BHC16 + (1|JDAY) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois16)

pois17 <- glmmTMB(richness ~ MonYear + xEasting + (1|JDAY) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois17)

pois18 <- glmmTMB(richness ~ MonYear + xEasting + BHC16 + (1|JDAY) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois18)

pois19 <- glmmTMB(richness ~ MonYear + xEasting + BHC16 + (1|JDAY) + (1|TSSR) + (1|Station), data = divdat, family = (poisson("log")))
summary(pois19)
...
e <- predictorEffect("BHC16", bestmod)
plot(e)
...
pois18 <- glmer(richness ~ MonYear + xEasting + BHC7 + (1|Station), data = divdat, family = (poisson("log")), 
...
# Set effect size at 20% over 10 years. Annual change of 0.1
fixef(msim)["MonYear"]
fixef(msim)["MonYear"]<- -.0224
summary(msim)

# Determine how many years of monitoring required to detect 20% change
msim.extend <- extend(msim, along = "MonYear", n=10)
summary(msim.extend)
years.pc <- powerCurve(msim.extend, nsim = 75, alpha = 0.05, breaks = c(3,4,5,6,8,10))
years.out <- summary(years.pc)
years.out

write_csv(years.out, path = paste("C:/Users/jeff.matheson/Documents/R Working/SiteC/Song/PowerOut/abun-power-yearsout.csv", 
                               sep=""))

years.plot <- ggplot(years.out, aes(x=nlevels, y=mean))+
  geom_line()+ geom_point()+ xlab(label = "Monitoring Year")+ ylab(label = "Power")+
  labs(subtitle="Mean Station Abundance")+
...
ggsave(paste("C:/Users/jeff.matheson/Documents/R Working/SiteC/Song/PowerOut/Abun-power-years.png", sep=""), 
       years.plot)

# Assuming 10 years of monitoring, what is min number of stations detect 20% change/year?
...
write_csv(stations.out, path = paste("C:/Users/jeff.matheson/Documents/R Working/SiteC/Song/PowerOut/abun-power-stationsout.csv", sep=""))
...
stations.plot <- ggplot(stations.out, aes(x=nlevels, y=mean))+
  ...
  annotate(geom="text", x=175, y=0.35, label="Power to detect 20% change in 10 years",
         color="black")+
  annotate(geom="text", x=175, y=0.30, label="at vaying number of survey stations",
           color="black")
stations.plot

ggsave(paste("C:/Users/jeff.matheson/Documents/R Working/SiteC/Song/PowerOut/abun-power-stations-20-05.png", sep=""), stations.plot)
